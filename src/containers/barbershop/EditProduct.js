...
const DATE = new Date();
const EditProductForm = ({ submit, closeModal, visible, id, name, price, description, hours, minutes }) => {
    DATE.setHours(hours);
    ...
    const preSubmit = () => {
        closeModal()
        submit(id, nameForm, priceForm, descriptionForm, delayForm.getHours(), delayForm.getMinutes())
    }
    return (
                ...
                <View style={styles.modal}>
                    ...
                    <TouchableOpacity style={styles.btnDelay} onPress={showTimepicker}>
                        <Text style={{ color: LIGHT_COLOR, textAlign: 'center' }}>
                            {"Demora " + delayForm.getHours() + ":" + (delayForm.getMinutes() || "00") + "hs"}
                        </Text>
                    </TouchableOpacity>
                    {show && (
                        <DateTimePicker
                    ...
                    <View style={styles.rowButtons}>
                        <Button
                            ...
                            buttonStyle={{ backgroundColor: DARK_COLOR, borderRadius: hp('2.5%'), paddingHorizontal: wp('3.5%') }}
                        />
                        <Button
                            onPress={preSubmit}
                            title="Guardar"
                            buttonStyle={{ backgroundColor: PRIMARY_COLOR, borderRadius: hp('2.5%'), paddingHorizontal: wp('3.5%') }}
                        />
                    </View>
                </View>
            </View>
