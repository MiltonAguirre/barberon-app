...
const MONTHS = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiember', 'Octubre', 'Noviembre', 'Diciembre'];
const MONTHS_SHORT = ['ENE', 'FEB', 'MAR', 'ABR', 'MAY', 'JUN', 'JUL', 'AGO', 'SEP', 'OCT', 'NOV', 'DIC'];

const ModalTurn = ({ closeModal, visible, state, start, barbershop, product, user, imBarber, id, cancelTurn, confirmTurn, ending }) => {
    const date = new Date(start);
    const dateTurn = DAYS[date.getDay()] + ' ' + date.getDate() + ' ' + MONTHS_SHORT[date.getMonth()] + ' ' + date.getFullYear().toString().slice(-2)
            ...
            <View style={styles.modalContent}>
                <View style={styles.modal}>
                    <View style={[styles.row, { justifyContent: 'space-between' }]}>
                        ...
                        <Icon name='close' size={25} color={DARK_COLOR} onPress={closeModal} />
                    </View>
                        ...
                        imBarber
                            ...
                            <View style={styles.info}>
                                <View style={styles.row}>
                                    ...
                                    />
                                    <Text style={styles.textName}>{user.data_user.first_name + ' ' + user.data_user.last_name}</Text>
                                </View>
                                <View style={styles.row}>
                                    <Icon
                                        name='mail'
                            ...
                            <View style={styles.info}>
                                ...
                                <View style={styles.row}>
                                    <Icon
                                        ...
                                        iconStyle={{ paddingRight: 10 }}
                                    />
                                    <Text style={styles.text}>{barbershop.location.address + ' (CP: ' + barbershop.location?.zip + ') ' + barbershop.location?.country}</Text>
                                </View>
                                <View style={styles.row}>
                                    <Icon
                                        name='location-city'
                                        ...
                                        iconStyle={{ paddingRight: 10 }}
                                    />
                                    <Text style={styles.text}>{barbershop.location?.city + " - " + barbershop.location?.state}</Text>
                                </View>
                            </View>
